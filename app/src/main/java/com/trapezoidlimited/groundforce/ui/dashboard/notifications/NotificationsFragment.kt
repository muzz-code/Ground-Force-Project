...
class NotificationsFragment : Fragment() {
    ...
    private val adapterNew = NotificationsAdapter(mutableListOf())
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val recyclerViewNew = binding.notificationsFragmentRecyclerOldNotificationsRv
        ...
        context?.let {
            recyclerView.addItemDecoration(
                DividerItemDecoration(ContextCompat.getColor(it, R.color.colorTextDidntGetText), heightInPixels)
            )
        }
    ...
    private fun returnNotifications(num: Int): List<NotificationsItem>{
        ...
        for (nu in 0..num){
            ...
            when(nu % 4){
                ...
                2 -> notification.add(notificationOne)
                else -> notification.add(notificationTwo)
            }
        }
    ...
    override fun onResume() {
        super.onResume()
      activity?.let {
          val lists = returnNotifications(1)
        ...
        activity?.let {
            val lists = returnNotificationsNew(7)

            Log.d("CHECKINGSLIST", "$lists")
            lists.let {
                adapterNew.updateNotifications(it)
            }
        }
    }

    private fun returnNotificationsNew(num: Int): List<NotificationsItem>{
        val notifications = mutableListOf<NotificationsItem>()
        val isNew = mutableListOf<String>("New Notification","Older Notifications")
        val notification = mutableListOf<NotificationItem>()

        val notificationOne = NotificationItem(image = R.drawable.bolt, message = "Hello Trooper Kehinde, a credit transaction of NGN100.00 just occurred in your account. Login to review. If there are any complaints or inquiries, please feel free to contact us.", date = "13 Sept, 2020 10:30 AM" , isNew = "Older Notification")
        val notificationThree = NotificationItem(image = R.drawable.bolt, message = "Hello Trooper Kehinde, a credit transaction of NGN100.00 just occurred in your account. Login to review. If there are any complaints or inquiries, please feel free to contact us.", date = "13 Sept, 2020 10:30 AM" , isNew ="Older Notifications")
            ...
            when(nu % 4){
                ...
                else -> notification.add(notificationThree)
            }
        }
